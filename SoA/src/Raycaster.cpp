...
bool RaycastGround(const glm::vec3& rayOrigin, const glm::vec3& rayDir, glm::vec3& outIntersection) {
    if (std::abs(rayDir.y) < 1e-6f)
        return false; // Ray is parallel to the ground.
                   ...
                   glm::vec3& outNormal)
{
    ...
    for (size_t i = 0; i < soA.position.size(); i++) {
        ...
        if (t >= 0.f && t < nearestT) {
            // This block is the closest intersection so far.
            nearestT = t;
            outNormal = boxNormal;
            hitAny = true;
        }
    ...
    if (hitAny) {
        // We clamp nearestT to maxDistance, so no infinite range placing
